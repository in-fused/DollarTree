...
function openStoreModal(storeId) {
  ...
  currentSelectedStoreId = normalizedStoreId;
...
async function updateStore(storeId, completedOrStatus, closed = false, statusReason = "") {
  ...
  const normalizedStoreId = String(storeId);
  const nextStatus = completedOrStatus && typeof completedOrStatus === "object"
  ...
  const existingStatusRowIndex = statusRowsCache.findIndex(row => String(row.store_id) === normalizedStoreId);
  const nextStatusRow = {
    ...(existingStatusRowIndex >= 0 ? statusRowsCache[existingStatusRowIndex] : {}),
    project_id: currentProjectId,
    store_id: normalizedStoreId,
    completed: nextStatus.completed,
    closed: nextStatus.closed,
    status_code: nextStatus.status_code,
    status_reason: nextStatus.status_reason,
    updated_at: new Date().toISOString()
  };

  if (existingStatusRowIndex >= 0) {
    statusRowsCache.splice(existingStatusRowIndex, 1, nextStatusRow);
  } else {
    statusRowsCache.unshift(nextStatusRow);
  }
